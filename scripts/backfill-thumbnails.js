...
//     needed).
//  2. Thumbnail generation: writes a resized JPEG to storage_path's `thumbs/`
//     sibling and records it in `photos.thumb_url`, so grid views (CatchGrid/
//     SearchPage) can request a lighter image than the full photo. Live
//     uploads (src/lib/fileLoader.js) now generate this thumbnail themselves —
//     this script exists for backfilling photos uploaded before that landed.
//
// THUMB_MAX_PX/THUMB_QUALITY were bumped 2026-07-15 (400px/72 → 960px/82) after
// the original 400px thumbnails, generated by this script, turned out too small
// for the current ~450-550px CatchGrid cards on retina displays and rendered
// blurry. Pass --force to regenerate thumbnails that already exist (needed once
// to replace the undersized ones); without it, only missing thumbnails are filled in.
//
// Requires: npm install --save-dev sharp (already a devDependency)
// Run: node scripts/backfill-thumbnails.js [--force]
...
async function main() {
  ...
  for (const photo of photos) {
    try {
      ...
      const needsThumb = FORCE || !photo.thumb_url
